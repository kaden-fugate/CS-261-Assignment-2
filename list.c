...
{ 
  ...
  struct node *temp = list->head; // initial vals
  int count = 0; 

  while(temp){ temp = temp->next; count++; } // while temp exists, increment count
...
/*
 * this function will print a linked list
 * 
 * Params:
 *      list - list to be printed
 *      p - function to print caller
 */
void print_list(struct list *list, void (*p)(void *))
{

  struct node *temp = list->head; // init val
  
  while(temp){

    (*p)(temp->val); // print val
    temp = temp->next;
...
/*
 * this function returns 1 if list if empty, 0 if not
 *
 * Param:
 *      list - list to be checked
 */
int head_exists(struct list *list)
{
...
/*
 * this function acts as a peek function for a stack
 *
 * Param:
 *      list - list to be peeked on 
 */
void* list_head(struct list *list)
{

  return list->head->val; // head val acts as top of stack

}

/*
 * this function pops the top of the stack off. popped val is returned
 * 
 * Param:
 *      list - list of the stack to be popped
 */
void shift_l_head(struct list *list)
{

  struct node *temp = list->head; // head acts as top of stack

  list->head = temp->next; // shift head and delete old head
  free(temp);
