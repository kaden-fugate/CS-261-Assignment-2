...
struct queue_from_stacks* queue_from_stacks_create() 
{
	...
	// allocate mem for qfs
	struct queue_from_stacks *temp = malloc( sizeof( struct queue_from_stacks ) );

	// init stacks
	temp->s1 = stack_create(), temp->s2 = stack_create();
...
void queue_from_stacks_free(struct queue_from_stacks* qfs) 
{
	...
	stack_free(qfs->s1), stack_free(qfs->s2);
...
int queue_from_stacks_isempty(struct queue_from_stacks* qfs) 
{
	...
	if(stack_isempty(qfs->s1) && stack_isempty(qfs->s2)){ return 1; } // if both stacks empty, queue is empty
...
void queue_from_stacks_enqueue(struct queue_from_stacks* qfs, void* val) 
{
	...
	if( queue_from_stacks_isempty(qfs) ) // if queue is already empty, add first val
	{ 
	...
	while( !stack_isempty(qfs->s1) ){ stack_push(qfs->s2, stack_pop(qfs->s1)); } // transfer s1 to s2

	stack_push(qfs->s1, val); // add new val to bottom of stack

	while( !stack_isempty(qfs->s2) ){ stack_push(qfs->s1, stack_pop(qfs->s2)); } // transfer s1 to s2
