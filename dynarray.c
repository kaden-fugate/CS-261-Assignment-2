...
/*
 * this function should just return the size of the array (size - head)
 * 
 * Param:
 *      arr - array
 */
int arr_size(struct dynarray *arr){ return (arr->size - arr->head); }

/*
 * this function will print the whole array starting at the head
 * 
 * Params:
 *      arr - array to print
 *      p - function to print caller struct
 */
void print_arr(struct dynarray *arr, void (*p)(void *))
{

  for(int i = arr->head; i < arr->size; i++){ (*p)(arr->data[i]); } // print vals 

}

/*
 * this function should return 1 if array empty, 0 if not
 * 
 * Param:
 *      arr - array to check
 */
int is_empty(struct dynarray *arr)
{ 
  
  if(arr->head == arr->size){ return 1; } 
...
/*
 * this function acts as a dequeue function for the queue and will
 * return the dequeued val
 *
 * Param:
 *      arr - array to dequeue val from
 */
void shift_a_head(struct dynarray *arr)
{

  arr->head++; // shift head val

}

/*
 * this function will return the value at the front of the array
 * 
 * Param:
 *      arr - array to peek
 */
void* arr_head(struct dynarray *arr)
{

  if( is_empty(arr) ){ return NULL; } // if arr->size 0, return NULL

  return *(arr->data + arr->head); // else, return de-referenced data at first index
